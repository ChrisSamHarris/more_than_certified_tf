output "repository_name_urls" {
  ...
  value       = { for i in values(github_repository.mtc_repo) : i.name => { "SSH URL" : i.ssh_clone_url, "Repo URL" : i.http_clone_url, "GH Pages URL" : try(i.pages[0].html_url, "No GH Pages configured") } }
...
output "github_pages_link_public" {
  value       = try(github_repository.mtc_repo["dev"].pages[0].html_url, "No dev environment found.")
